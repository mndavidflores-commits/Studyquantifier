...
// Almacena los errores pendientes actuales para el selector del modo B
...
function updatePomoButtons() {
  ...
  // Botón flotante de pausa/reanudación (toggle)
  ...
  if (isActiveSession) {
    if (st === State.FOCUS_RUNNING || st === State.BREAK_RUNNING) {
      floatPause.textContent = '⏯';   // icono de pausa
    } else {
      floatPause.textContent = '▶';    // icono de reanudar
    }
  }
...
document.getElementById('btnPomoPauseFloat').addEventListener('click', () => {
  // Toggle: pausar si está corriendo, reanudar si está pausado
...
function stopBlindTimerAndShowResult() {
  ...
  // Solo actualizar previousProblemaNum si NO estamos en modo B
  if (session.modo !== 'B') {
  ...
  // Ocultar panel izquierdo durante el formulario de resultado
  document.getElementById('left-panel').classList.add('hidden');
...
document.getElementById('btnSiguienteProblema').addEventListener('click', async () => {
  ...
  // Incrementar número solo si no es modo B
  if (modo !== 'B') {
...
document.getElementById('btnDescartarProblema').addEventListener('click', () => {
  // Restaurar número solo si no es modo B
  if (session.modo !== 'B') {
...
// Listener único para el selector de problemas pendientes (se añade una sola vez)
...
async function mostrarColaErrores() {
  ...
  erroresPendientes = errores; // actualizar variable global
  ...
  if (errores.length === 0) {
    ...
    // No añadimos más listeners; el global ya maneja el cambio
...
document.getElementById('btnGuardarRepaso').addEventListener('click', async () => {
  ...
  actualizarHistorialSubtema();
  document.getElementById('conjeturas-sesion-wrap').style.display = 'block';
  // Restaurar panel izquierdo después del repaso
  document.getElementById('left-panel').classList.remove('hidden');
  actualizarConjeturasSesion();
});
