...
async function corregirSesionId(tempId, idSesionReal) {
  ...
  async function transition(newState) {
    ...
    if (newState === State.IDLE) {
      ...
      document.getElementById('filaNumProblema').style.display = 'block';
      document.getElementById('colaErroresWrap').style.display = 'none';
      document.getElementById('formResultadoA').style.display = 'block';
      document.getElementById('formResultadoB').style.display = 'none';
  ...
  async function mostrarColaErrores() {
    ...
    cont.querySelectorAll('.error-queue-item').forEach(el => {
      el.addEventListener('click', () => {
        cont.querySelectorAll('.error-queue-item').forEach(x => x.classList.remove('selected'));
        el.classList.add('selected');
        errorSeleccionado = errores.find(e => e.id === el.dataset.errorId);
      });
    });
  }

  // Alterna la UI de "Temporizador ciego" / "Resultado" entre el flujo A (numProblema + form
  // completo) y el flujo B (cola de errores + form reducido). Modo C no se toca.
  async function actualizarUIPorModo() {
    const esModoB = session.modo === 'B';
    document.getElementById('filaNumProblema').style.display = esModoB ? 'none' : 'block';
    document.getElementById('colaErroresWrap').style.display = esModoB ? 'block' : 'none';
