...
import { fsrs, generatorParameters, createEmptyCard, Rating, State as EstadoFSRS } from 'https://cdn.jsdelivr.net/npm/ts-fsrs@5.4.1/+esm';
  ...
  let historialSubtemaCache = [];
  let detalleProblemaActual = null;
  const blindTimer = {
...
let appInitialized = false;
  function actualizarUI(s) {
    ...
    if (s?.user) {
      ...
      document.getElementById('headerUserEmail').textContent = s.user.email;
...
const { data: { session: s } } = await supabase.auth.getSession();
  ...
  document.getElementById('btn-logout-icon').addEventListener('click', async () => {
...
data: { ...c, sesion_id: idSesionReal, user_id: sessionActual.user.id, updated_at: new Date().toISOString() },
  ...
  // ==================== NUEVO: coreografía de zonas visuales de la sesión ====================
  // Mueve los controles reales del pomodoro (mismos elementos, mismos listeners) a la píldora
  // flotante cuando arranca la sesión, y los devuelve a la tarjeta en reposo al terminar.
  const elementosFlotantes = ['pomoCircle', 'pomoControls', 'filaLectura', 'btnDistraje'];
  function moverAFlotante() {
    const bar = document.getElementById('pomoBarFlotante');
    elementosFlotantes.forEach(id => { const el = document.getElementById(id); if (el) bar.appendChild(el); });
  }
  function moverARestingCard() {
    const anchor = document.getElementById('pomoControlsAnchor');
    const grid = document.getElementById('pomoWorkBreakGrid');
    const card = document.getElementById('cardPomodoro');
    const circle = document.getElementById('pomoCircle'), controles = document.getElementById('pomoControls');
    const lectura = document.getElementById('filaLectura'), distraje = document.getElementById('btnDistraje');
    if (circle) anchor.insertBefore(circle, anchor.firstChild);
    if (controles) anchor.appendChild(controles);
    if (lectura) grid.appendChild(lectura);
    if (distraje) card.appendChild(distraje);
  }
  function fadeIn(el) {
    if (!el) return;
  ...
  // Interruptor grueso: dentro de sesión (pomodoro corriendo/pausado) vs. en reposo.
  function actualizarVisibilidadSesion() {
    const enSesion = session.state !== State.IDLE && session.state !== State.SESSION_ENDING;
    document.body.classList.toggle('session-active', enSesion);
    document.getElementById('cardConfigSesion').style.display = enSesion ? 'none' : 'block';
    document.getElementById('cardPomodoro').style.display = enSesion ? 'none' : 'block';
    document.getElementById('cardTimer').style.display = enSesion ? 'block' : 'none';
    document.getElementById('cardConjeturas').style.display = enSesion ? 'block' : 'none';
    if (!enSesion) {
      document.getElementById('cardHistorialSubtema').style.display = 'none';
      document.getElementById('pomoBarFlotante').style.display = 'none';
      document.getElementById('cardResultado').style.display = 'none';
    }
  }
  // Interruptor fino: dentro de la sesión, según el cronómetro de problema esté en
  // selección / corriendo / formulario abierto (ver protocolo_estudio_v2 y la spec de UI).
  function actualizarZonaTimer() {
    const corriendo = blindTimer.running;
    const formularioAbierto = blindTimer.pendingResult;
    const inactivoTimer = !corriendo && !formularioAbierto;
    const modoB = session.modo === 'B';

    const historial = document.getElementById('cardHistorialSubtema');
    const yaVisible = historial.style.display === 'block';
    historial.style.display = inactivoTimer ? 'block' : 'none';
    if (inactivoTimer && !yaVisible) fadeIn(historial);

    const bar = document.getElementById('pomoBarFlotante');
    if (session.state !== State.IDLE && session.state !== State.SESSION_ENDING) {
      bar.style.display = corriendo ? 'none' : 'flex';
    }

    document.getElementById('filaNumProblema').style.display = (inactivoTimer && !modoB) ? 'block' : 'none';
    document.getElementById('filaSelErrorPendiente').style.display = (inactivoTimer && modoB) ? 'block' : 'none';
    document.getElementById('filaTipoProblema').style.display = inactivoTimer ? 'block' : 'none';
    document.getElementById('timerControls').style.display = inactivoTimer ? 'flex' : 'none';

    document.getElementById('cardTimer').style.display = formularioAbierto ? 'none' : 'block';
    document.getElementById('cardConjeturas').style.display = formularioAbierto ? 'none' : 'block';
    const listaConj = document.getElementById('listaConjeturasSesion');
    if (listaConj) listaConj.style.display = corriendo ? 'none' : 'block';

    document.getElementById('cardResultado').style.display = formularioAbierto ? 'block' : 'none';
  }
  function registrarDistraccion() {
  ...
  // ==================== FIN NUEVO: coreografía de zonas visuales ====================
  ...
  document.getElementById('btnDistraje').addEventListener('click', registrarDistraccion);
  ...
  // Atajos de teclado: barra espaciadora inicia/detiene el cronómetro ciego, tecla "d" registra
  // una distracción. Ambos se ignoran si el foco está en un campo de texto (para no interferir
  // con escribir, p.ej., una conjetura) o si hay un modal abierto.
  function modalAbiertoAlguno() {
    return document.getElementById('modalResumen').style.display === 'flex' ||
           document.getElementById('modalDetalleProblema').style.display === 'flex';
  }
  document.addEventListener('keydown', e => {
    if (document.activeElement !== document.body) return;
    if (modalAbiertoAlguno()) return;
    if (e.code === 'Space') {
      e.preventDefault(); // evita que la página haga scroll con la barra espaciadora
      if (e.repeat) return; // ignora la autorrepetición mientras se mantiene presionada
      if (blindTimer.running) stopBlindTimerAndShowResult();
  ...
  document.addEventListener('keyup', e => {
    ...
    if (blindTimer.pendingResult) return; // formulario abierto: el espacio no hace nada
    if (!blindTimer.running) startBlindTimer();
  });
...
nivel_bloom: parseInt(document.getElementById('selBloom').value), sesion_id: session.tempId
  ...
  // ==================== NUEVO: Historial de problemas por subtema (panel izquierdo) ====================
  // Muestra solo los problemas del modo activo de la sesión (A, B o C), para no mezclar
  // desempeño de otras condiciones en el panel visible durante, por ejemplo, una sesión modo A.
  async function actualizarHistorialSubtema() {
    const cont = document.getElementById('cardHistorialSubtemaBody');
    if (!cont) return;
    const materia = document.getElementById('selMateria').value;
    const subtema = document.getElementById('selSubtema').value;
    if (!materia || !subtema || materia === '__agregar__' || subtema === '__agregar__') {
      cont.innerHTML = '<p style="color:var(--text2);">Sin subtema seleccionado.</p>';
      return;
    }
    const modo = session.modo || document.getElementById('selModo').value;
    const registros = (await db.sessions.where('tipo').equals('problema').toArray())
      .filter(s => s.materia === materia && s.subtema_id === subtema && s.modo === modo)
      .sort((a, b) => (a.timestamp || 0) - (b.timestamp || 0));
    historialSubtemaCache = registros;
    if (!registros.length) { cont.innerHTML = '<p style="color:var(--text2);">Sin problemas registrados aún en este modo.</p>'; return; }
    const badge = r => r === 'bien' ? '<span class="res-badge res-bien">B</span>' : r === 'mal' ? '<span class="res-badge res-mal">M</span>' : '<span class="res-badge res-nr">NR</span>';
    cont.innerHTML = '<table><tr><th>#</th><th>Tiempo</th><th>Resultado</th></tr>' +
      registros.map(r => `<tr data-id="${r.id}"><td>${r.problema_num ?? '-'}</td><td>${formatTime(r.tiempo_s || 0)}</td><td>${badge(r.resultado)}</td></tr>`).join('') +
      '</table>';
    cont.querySelectorAll('tr[data-id]').forEach(tr => {
      tr.addEventListener('click', () => abrirDetalleProblema(tr.dataset.id));
    });
  }
  async function abrirDetalleProblema(id) {
    const r = await db.sessions.get(id);
    if (!r) return;
    detalleProblemaActual = r;
    document.getElementById('detalleProblemaContenido').innerHTML = `
      <p><strong>Fecha:</strong> ${r.fecha || '-'}</p>
      <p><strong>Problema:</strong> #${r.problema_num ?? '-'}</p>
      <p><strong>Tiempo:</strong> ${formatTime(r.tiempo_s || 0)}</p>
      <p><strong>Resultado:</strong> ${r.resultado || '-'}</p>
      <p><strong>Confianza:</strong> ${r.confianza ?? '-'}</p>
      <p><strong>Dificultad:</strong> ${r.dificultad_experimentada ?? '-'}</p>
      <p><strong>Bloom:</strong> ${r.nivel_bloom ?? '-'}</p>
      <p><strong>Intentos:</strong> ${r.intentos ?? '-'}</p>
      <p><strong>Error:</strong> ${r.codigo_error ?? '-'}</p>
      <p><strong>Condición:</strong> ${r.fase ?? '-'}</p>
    `;
    document.getElementById('modalDetalleProblema').style.display = 'flex';
  }
  document.getElementById('btnCerrarDetalleProblema').addEventListener('click', () => {
    document.getElementById('modalDetalleProblema').style.display = 'none';
    detalleProblemaActual = null;
  });
  document.getElementById('btnEliminarDetalleProblema').addEventListener('click', async () => {
    if (!detalleProblemaActual) return;
    if (!confirm('¿Eliminar este registro? Si generó un error en seguimiento FSRS, también se eliminarán ese error y sus repasos. Esta acción no se puede deshacer.')) return;
    const id = detalleProblemaActual.id;
    await db.sessions.delete(id);
    await db.outbox.put({ table: 'study_sessions', record_id: id, operation: 'delete', data: { id, user_id: sessionActual.user.id, deleted_at: new Date().toISOString() }, onConflict: 'id', created_at: new Date().toISOString() });
    // Cascada: si este problema generó un error de seguimiento FSRS, se elimina junto con sus repasos,
    // para no dejar en la cola de modo B un error "huérfano" que apunte a un problema inexistente.
    const erroresVinculados = (await db.errores.toArray()).filter(e => e.sesion_id_origen === id);
    for (const err of erroresVinculados) {
      const repasosVinculados = (await db.repasos.toArray()).filter(r => r.error_id === err.id);
      for (const rep of repasosVinculados) {
        await db.repasos.delete(rep.id);
        await db.outbox.put({ table: 'repasos', record_id: rep.id, operation: 'delete', data: { id: rep.id, user_id: sessionActual.user.id, deleted_at: new Date().toISOString() }, onConflict: 'id', created_at: new Date().toISOString() });
      }
      await db.errores.delete(err.id);
      await db.outbox.put({ table: 'errores', record_id: err.id, operation: 'delete', data: { id: err.id, user_id: sessionActual.user.id, deleted_at: new Date().toISOString() }, onConflict: 'id', created_at: new Date().toISOString() });
    }
    await syncAll();
    document.getElementById('modalDetalleProblema').style.display = 'none';
    detalleProblemaActual = null;
  ...
  // ==================== FIN NUEVO: Historial de problemas por subtema ====================
  ...
  // ==================== NUEVO: MOTOR FSRS + DOMINIO DE TEMAS ====================

  // ---- Configuración FSRS-6 (retención objetivo fija 0.9; sin pasos de aprendizaje en minutos,
  // porque esta app revisa entre sesiones de estudio, no dentro de la misma sesión) ----
  ...
  // Punto 2: pesos congelados por materia. Se crean con los pesos por defecto de la librería
  // la primera vez que una materia genera un error, y de ahí en adelante siempre se reutilizan.
  ...
  // Reconstruye el objeto Card de ts-fsrs a partir de una fila de `errores`.
  // No persistimos elapsed_days/scheduled_days/learning_steps/lapses porque ts-fsrs los
  // recalcula internamente (o, en el caso de lapses, no interviene en las fórmulas de
  // estabilidad/dificultad de FSRS-6 — solo es un contador informativo).
  ...
  // dias_desde_repaso_anterior (punto 4): fecha del repaso anterior de ESE error_id,
  // o fecha_creacion si todavía no tiene repasos.
  ...
  // guardarLocalYOutbox solo persiste los campos que le pasas (no hace merge con la fila
  // existente), así que para actualizar SOLO un par de campos de `errores` primero hay que
  // traer la fila completa y fusionar — si no, se pierden materia/subtema/etiqueta/etc.
  ...
  // Punto 5: disparador de creación, SOLO modo A + resultado mal/no_resuelto. Usa el estado
  // inicial que entrega createEmptyCard() de la librería, tal cual.
  async function crearErrorDesdeProblema({ materia, subtemaId, subtemaNombre, etiqueta, fase, idProblema }) {
    await getPesosCongelados(materia); // asegura que existan pesos congelados para esta materia
  ...
  // Punto 6: cola de errores activos, vencidos hoy o antes, para la materia/subtema de la sesión.
  // Se muestra como un menú desplegable (en vez de una lista de tarjetas clicables).
  async function mostrarColaErrores() {
    ...
    const sel = document.getElementById('selErrorPendiente');
    errorSeleccionado = null;
    if (!errores.length) {
      sel.innerHTML = '<option value="">Sin errores pendientes</option>';
      sel.disabled = true;
      return;
    }
    sel.disabled = false;
    ...
    sel.innerHTML = '<option value="">Selecciona un problema…</option>' + errores.map((e, i) =>
      `<option value="${e.id}">Problema #${numerosProblema[i]} · ${e.etiqueta || '—'} · vence ${new Date(e.proxima_revision).toLocaleDateString()}</option>`
    ).join('');
  }
  document.getElementById('selErrorPendiente').addEventListener('change', async function () {
    if (!this.value) { errorSeleccionado = null; return; }
    errorSeleccionado = (await db.errores.get(this.value)) || null;
  });

  // Alterna la UI de "Temporizador" / "Resultado" entre el flujo A (numProblema + form
  // completo) y el flujo B (dropdown de errores + form reducido). Modo C no se toca.
  ...
  // Punto 6, guardado en modo B: llama a FSRS con el estado reconstruido + la calificación,
  // guarda el repaso, actualiza el error, y además inserta una fila normal en 'sessions' para
  // que los paneles de métricas existentes sigan funcionando sin tocarlos.
...
resultado: calificacion >= 3 ? 'bien' : 'mal',
  ...
  // Descartar un intento cronometrado en modo B sin guardarlo (agregado por simetría con
  // btnDescartarProblema; no estaba pedido explícitamente). Mantiene errorSeleccionado para
  // poder reintentar el mismo error.
  ...
  // ---- Punto 9: registro de dominio, independiente del pomodoro activo ----
  ...
  // Punto 9: lógica de dos etapas para declarar dominio.
  // NOTA: la especificación no cubre explícitamente el caso "diferida ≥9 sin que la inmediata
  // ya esté aprobada" (p.ej. si se registra la diferida antes que la inmediata, o si la
  // inmediata previa fue <9). Aquí ese caso guarda resultado_diferida pero NO marca
  // dominio_alcanzado — queda pendiente. Confírmame si prefieres otro comportamiento.
  ...
  // ==================== FIN NUEVO: MOTOR FSRS + DOMINIO DE TEMAS ====================
