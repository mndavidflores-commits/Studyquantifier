...
export async function corregirSesionId(tempId, idSesionReal) {
  ...
  for (const p of problemas) {
    await db.outbox.put({
      ...
      record_id: p.id,
      operation: 'insert',
      data: { ...p, sesion_id: idSesionReal, user_id: state.sessionActual.user.id, updated_at: new Date().toISOString() },
      onConflict: 'id',
      created_at: new Date().toISOString()
    });
  }
  ...
  for (const c of conjs) {
    await db.outbox.put({
      ...
      record_id: c.id,
      operation: 'insert',
      data: { ...c, sesion_id: idSesionReal, user_id: state.sessionActual.user.id, updated_at: new Date().toISOString() },
      onConflict: 'id',
      created_at: new Date().toISOString()
    });
  }
